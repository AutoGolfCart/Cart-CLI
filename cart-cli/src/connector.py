...
class CLI_Connector:
    ...
    def __init__(self, ip_addr, establish_port=42024, command_port=69, log_port=420, response_port=777):
        # Create Ports
        ...
        # commands Logger (Useful for training and repeating actions)
        self.commands_logger = logging.getLogger("commands")
        self.commands_logger.setLevel(logging.DEBUG)
        commands_file_handler = logging.FileHandler("recored_commands.log")
        commands_file_handler.setFormatter("%(asctime)s - %(message)s")
        self.commands_logger.addHandler(commands_file_handler)
        ...
        response_console_handler.setFormatter("%(message)s")
    ...
    def establish_connection(self, max_attempts = 5):
        ...
        while not connected:
            ...
            # Send request
            s.send(("Bruh, I want to command you!").encode())
            data = s.recv(1024)
